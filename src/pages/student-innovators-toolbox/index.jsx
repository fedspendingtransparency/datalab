...
import rolesResponsibilities
	from '../../page-sections/student-innovators-toolbox/roles-responsibilities';
...
class AnalystGuidePage extends Component {
	render() {
		...
		const introSentence = (
			<>
				<p>
					The Student Innovator’s Toolbox was designed to connect students and
					faculty with data from the U.S. Treasury. The purpose is to complement
					classroom learning about the federal budget, data science, policy analysis
					as well as facilitation of analysis and research.
				</p>
				<p>
					The intended audience for this toolbox includes professors and instructors,
					directors of graduate and undergraduate programs, librarians dealing with
					data analysis and visualization, and students looking for a real-world
					project to flex their skills and develop data-driven solutions.
				</p>
			</>
		);
		const rawContent = [
		...
		const nonAccordionContent = [projects];
				...
				{rawContent.map((section, key) => {
					return (
						<section key={key} className={styles.section}>
							...
							{section.introSentence ? section.introSentence : ''}
							{section.data.map((datum, key1) => {
								let accordionTitle = <></>;
								let accordionContent = <></>;
								if (datum.accordionTitles && datum.accordionTitles.length) {
									accordionTitle = (
										<div className={styles.containerNoFlex} key={key1}>
