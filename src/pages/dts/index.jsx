...
export default class DTSPage extends React.Component {
  ...
  getData = (pageStart) => {
    return axios.get(`https://www.transparency.treasury.gov/services/api/fiscal_service/v1/accounting/dts/dts_table_2?fields=record_date,transaction_catg,transaction_today_amt,transaction_mtd_amt,transaction_fytd_amt&page[size]=10000&page[number]=${pageStart}`)
      .then(resp => {
        returnArray = returnArray.concat(resp.data.data);
        console.log(returnArray);
        return this.getData(pageStart + 1);
      }).catch(() => {
        let mappedDTSData = returnArray.map(d => {
          return { date: d.record_date, item_raw: d.transaction_catg, mtd: d.transaction_mtd_amt, fytd: d.transaction_fytd_amt, today: d.transaction_today_amt };
        });
        console.log("mapped data", mappedDTSData);
        this.setState({dtsData: mappedDTSData});
        return returnArray;
      });
  };
