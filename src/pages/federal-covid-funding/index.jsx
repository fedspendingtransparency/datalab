...
const BudgetAccordionContent = () => (
	<div className={styles.accordionContents}>
		<p>
			...
			monthly basis, starting in July 2020. While the Treasury validates the data
			by comparing it to information from other systems, each federal agency’s
			Chief Financial Officer (CFO) is responsible for certifying all of the
			agency’s reporting, and ensuring the data is correct.
		</p>
		<p>
			For the COVID-19 special appropriations, agencies are required to include a
			special field called the <b>Disaster Emergency Fund Code (DEFC)</b> in their
			financial reporting. Data Lab is tracking COVID-19 appropriations through the
			spending lifecycle using the DEFC that can be used to map transactions to
...
export default class FederalCovidFunding extends React.Component {
	...
	sections = [
		{
			...
			introBlurb: [
				<>
					<p key="section-1-p-1">
						To aid the nation’s recovery from the coronavirus disease 2019 (COVID-19)
						pandemic, the U.S. Congress passed four special appropriations laws for
						the federal government to use in relief efforts. The largest of these was
						the Coronavirus Aid, Relief, and Economic Security (CARES) Act, which
						provides approximately ${CovidCopy.carestotal_round} trillion and is the
						largest supplemental appropriation in American history.
					</p>
					<p key="section-1-p-2">
						In this analysis, we explore the four COVID-19 supplemental appropriation
						laws passed by the U.S. Congress, starting with the amount of funding
						budgeted for agencies and how that money can provide financial relief.
						Next, we walk through the process of how the money moves from
						appropriations to individuals and businesses, and finally, we dive into
			...
			introBlurb: [
				<>
					<p key="section-2-p-1">
						...
						{CovidCopy.totalbudgetresources} trillion in supplemental funds through
						the full budget lifecycle to the American people. In this section, we
						follow the flow of money by breaking down a few key terms and explaining
						how agencies spend supplemental funding.
					</p>
					<p key="section-2-p-2">
					...
					<p key="section-2-p-3">
						Creating an obligation doesn’t mean the money has been paid, only that the
						agency has promised to pay the funds. In many cases, the recipients of
						funds may be required to do something before receiving payment, such as
			...
			introBlurb: [
				<>
					...
					<p key="section-3-p-2">
						...
						{CovidCopy.totobligations_trillions} trillion in obligations, of which $
						{CovidCopy.totoutlays_trillions} trillion was outlayed. These totals were
						calculated from agencies’ certified monthly reporting to the Treasury’s
	...
	Row({ index }) {
		const item = this.sections[index];
		const SectionName = this.sectionComponents[item.tagName];
		return (
			<StorySection key={`key-${index}`} header={this.sections[index]}>
				<SectionName
					sectionId={`section-${this.sections[index].anchor}`}
