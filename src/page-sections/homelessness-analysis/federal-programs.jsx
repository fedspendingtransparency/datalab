...
export default function FederalPrograms(props) {
  ...
  `);
	...
	let Tooltip;
	let mapTitle; let p21MapSvg; let contactPanel; let cocPanel; let p2MatrixSvg; let p21Path; let
		m;
    ...
    ;
		...
		d3.select('#p2_1_map')
			.append('desc')
			.attr('id', 'p2-map-desc')
			.text('Panel description of regional homelessness spending using 2018 Continuum of Care data. This panel is a searchable map with federal grant data.');
	}
   ...
   */
	...
	function BarChart(d) {
		...
		p23MatrixSvg.append('desc')
			.attr('id', 'p2-2-bar-desc')
			.text('This panel contains a bar graph of grant categories such as housing, education, employment, support services, and health.');
		...
		bar.attr('class', 'bar')
			...
			.style('fill', (d2) => {
				...
				if (d2.category === 'Health') { return '#F0CA4D'; }
				if (d2.category === 'Education') { return '#2A5DA8'; }
				if (d2.category === 'Support Services') { return '#E37B40'; }
				if (d2.category === 'Employment') { return '#F53855'; }
				return '#FFF';
			})
