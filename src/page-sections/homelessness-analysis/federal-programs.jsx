...
export default function FederalPrograms(props) {
	...
	if (
		data &&
		data.allCoCcfdAfunding2019V3Csv &&
		data.allCoCcfdAfunding2019V3Csv.nodes
	) {
	...
	function init() {
		...
		d3
			...
			.attr('id', 'p2-map-desc')
			.text(
				'Panel description of regional homelessness spending using 2018 Continuum of Care data. This panel is a searchable map with federal grant data.'
			);
	}
	...
	function MakeCoCTable(d7) {
		...
		for (let i = 0; i < tableData.length; i++) {
			if (tableData[i].coc_number === d7.properties.coc_number) {
				...
				return (
					'<table><tr><th></th><th class="table-heading">Individuals</th><th class="table-heading">Families</th><th class="table-heading">Total</th></tr>' +
					`<tr><td class="table-heading">Sheltered</td><td>${OtherformatNumber(
						tabDat.sheltered_total_homeless_individuals
					)}</td>` +
					`<td>${OtherformatNumber(
						tabDat.sheltered_total_homeless_people_in_families
					)}</td>` +
					`<td>${OtherformatNumber(tabDat.sheltered_homeless)}</td></tr>` +
					`<tr><td class="table-heading">Unsheltered</td><td>${OtherformatNumber(
						tabDat.unsheltered_homeless_individuals
					)}</td>` +
					`<td>${OtherformatNumber(
						tabDat.unsheltered_homeless_people_in_families
					)}</td>` +
					`<td>${OtherformatNumber(tabDat.unsheltered_homeless)}</td>` +
					`</tr><tr><td class="table-heading">Total</td><td>${OtherformatNumber(
						tabDat.homeless_individuals
					)}</td>` +
					`<td>${OtherformatNumber(tabDat.homeless_people_in_families)}</td>` +
					`<td>${OtherformatNumber(
						tabDat.total_homeless
					)}</td></tr></table><div class="item"><div class="header">Chronically Homeless:</div>` +
					`<div class="value">${OtherformatNumber(
						tabDat.chronically_homeless
					)}</div></div><div class="item"><div class="header">Homeless Veterans:</div>` +
					`<div class="value">${OtherformatNumber(
						tabDat.homeless_veterans
					)}</div></div><div class="item">` +
					'<div class="header">Average number of days in Emergency Shelters, Safe Havens, or Transitional Housing:</div>' +
					`<div class="value">${OtherformatNumber(
						tabDat.es_sh_th_avg_days
					)}</div></div>` +
					...
					`<div class="value">${OtherformatNumber(
						tabDat.total_persons_returns_in_6_mths
					)}</div></div><div class="item">` +
					`<div class="header">12 months</div><div class="value">${OtherformatNumber(
						tabDat.total_persons_returns_in_12_mths_should_include_the_6_month_cohort
					)}</div>` +
					'</div><div class="item"><div class="header">24 months</div>' +
					`<div class="value">${
						tabDat.total_persons_returns_in_24_mths_should_include_both_the_6_and_12_month_cohort ===
						'NA'
							? 'NA'
							: OtherformatNumber(
									tabDat.total_persons_returns_in_24_mths_should_include_both_the_6_and_12_month_cohort
	...
	function StateBarChart(d) {
		...
		for (let i = 0; i < cfdaLegendKeyValues.length; i++) {
			const k = cfdaLegend22.append('div').attr('id', 'p2_2_legend_key');
			k.append('div')
				.attr('id', 'p2_2_key')
			...
			const l = cfdaLegend.append('div').attr('id', 'p2_4_legend_key');
			l.append('div')
				.attr('id', 'p2_4_key')
				.style('position', 'relative')
				.append('svg')
		...
		const p24MatrixSvg = d3
			...
			.attr('aria-labelledby', 'p2-4-bar-desc');
		d3
			.select('#p2_4_info')
			.append('desc')
			.attr('id', 'p2-4-bar-desc')
			.text('This panel shows the federal programs serving the selected region.');
	...
	function makeContactTable(df) {
		return (
			`<h4 style="margin-bottom:0">${df.properties.COCNAME}</h4><br>` +
	...
	function makeInfographic(d) {
		...
		const selectedInforgraphicItem = inforgraphicData.filter(
			item => item.cluster_final === d
		);
		d3
			.select('#cluster-beds #count-img-container')
			.append('img')
			.attr('src', `${svgPath}/Bed.svg`)
			.attr('alt', clusterAltText[0].beds);
		d3
			.select('#cluster-circles')
			.append('img')
			.attr('src', `${svgPath}/Circles.svg`)
			.attr('alt', clusterAltText[0].funding);
		d3
			.select('#cluster-rent')
			.append('img')
			.attr('src', `${svgPath}/Rent.svg`)
			.attr('alt', clusterAltText[0].rent);
		d3
			.selectAll('.cluster-people')
			.append('img')
			.attr('src', `${svgPath}/People.svg`)
			.attr('alt', clusterAltText[0].homelessness);
		d3
			.selectAll('.cluster-population')
			.append('img')
			.attr('src', `${svgPath}/Total-Population.svg`)
			.attr('alt', clusterAltText[0].total_population);
		d3
			.select('#cluster-land-area')
			.append('img')
			.attr('src', `${svgPath}/Land-Area.svg`)
			.attr('alt', clusterAltText[0].land_area);
		d3
			.select('#cluster-income')
			.append('img')
			.attr('src', `${svgPath}/Income.svg`)
			.attr('alt', clusterAltText[0].income);
		d3
			.select('#cluster-key')
			.append('img')
			.attr('src', `${svgPath}/Key.svg`)
			.attr('alt', '');
		d3
			.selectAll('.cluster-density')
			.append('img')
			.attr('src', `${svgPath}/Density.svg`)
			.attr('alt', clusterAltText[0].density);
		d3
			.select('#cluster-rent-as-income')
			.append('img')
			.attr('src', `${svgPath}/Rent-as-Income.svg`)
			.attr('alt', clusterAltText[0].rent_as_income);
		d3.select('#cluster-number').text(d);
		d3
			.select('#cluster-beds-count')
			.text(OtherformatNumber(selectedInforgraphicItem[0].total_beds));
		d3
			.selectAll('.cluster-people-count')
			.text(OtherformatNumber(selectedInforgraphicItem[0].total_homeless));
	...
	function makeCoCTableTitle(d) {
		...
		return (
			`<p class="cocTabTitleCluster" style=color:white;background:${textColor}>Cluster ${d.cluster_final}: </p>` +
	...
	function makeCoCTableFund(d) {
		return (
			'<table className="FundingTable"><tr><th className="fundingTitle">' +
			'FEDERAL FUNDING FOR THE CONTINUUM OF CARE PROGRAM:</th></tr>' +
			`<tr><td className="fundingAmount">${formatNumber(
				d.CoC_program_funding
			)}</td></tr>` +
			'<tr><th className="fundingTitle">FEDERAL FUNDING FOR OTHER HOMELESSNESS PROGRAMS:</th></tr>' +
			`<tr><td className="fundingAmount">${formatNumber(
				d.Other_program_funding
			)}</td></tr></table>`
		);
	}
	...
	function makeCoCTableInfoCol2(d) {
		return (
			...
			`<tr><td className="infoAmount">${formatNumber(
				d.weighted_income
			)}</td></tr>` +
			'<tr><th className="fundingTitle">RENT: MEDIAN GROSS</th></tr>' +
			`<tr><td className="infoAmount">${formatNumber(
				d.weighted_estimate_median_gross_rent
			)}</td></tr>` +
			...
			`<tr><td class="infoAmount">${OtherformatNumber(
				d.land_area
			)}</td></tr></table>`
		);
	}
	...
	function p21Clicked(d) {
		...
		m.selectAll('p2_1_path').classed(
			'active',
			map2Centered && d === map2Centered
		);
		...
		m.transition()
			...
			.attr(
				'transform',
				`translate(${mapWidth / 1.35},${mapHeight /
					1.1})scale(${k})translate(${-x},${-y})`
			)
	...
	function p2GetColor(d) {
		for (let i = 0; i < tableData.length; i++) {
			if (d.properties.coc_number === tableData[i].coc_number) {
				...
				if (tableData[i].amount <= 1500000) {
					return '#B0EC9A';
				}
				if (tableData[i].amount <= 2500000) {
					return '#A3E59B';
				}
				if (tableData[i].amount <= 5000000) {
					return '#96DD9B';
				}
				if (tableData[i].amount <= 7500000) {
					return '#8AD59C';
				}
				if (tableData[i].amount <= 10000000) {
					return '#80CE9C';
				}
				if (tableData[i].amount <= 20000000) {
					return '#76C69C';
				}
				if (tableData[i].amount <= 30000000) {
					return '#6DBD9B';
				}
				if (tableData[i].amount <= 40000000) {
					return '#66B59A';
				}
				if (tableData[i].amount <= 50000000) {
					return '#5FAD98';
				}
				if (tableData[i].amount <= 60000000) {
					return '#5AA496';
				}
				if (tableData[i].amount <= 70000000) {
					return '#569C93';
				}
				if (tableData[i].amount <= 80000000) {
					return '#529490';
				}
				if (tableData[i].amount <= 90000000) {
					return '#508B8C';
				}
				if (tableData[i].amount <= 100000000) {
					return '#4E8387';
				}
				if (tableData[i].amount <= 150000000) {
					return '#465261';
				}
				if (tableData[i].amount <= 200000000) {
					return '#3E3C4A';
				}
			}
		}
	...
	function BarChart(d) {
		...
		p23MatrixSvg
			.append('desc')
			.attr('id', 'p2-2-bar-desc')
			.text(
				'This panel contains a bar graph of grant categories such as housing, education, employment, support services, and health.'
			);
		...
		bar
			...
			.style('fill', d2 => {
				...
				if (d2.category === 'Health') {
					return '#F0CA4D';
				}
				if (d2.category === 'Education') {
					return '#2A5DA8';
				}
				if (d2.category === 'Support Services') {
					return '#E37B40';
				}
				if (d2.category === 'Employment') {
					return '#F53855';
				}
				return '#FFF';
			})
	...
	function handleMapVizMouseOver(d) {
		const html =
			'<p style="border-bottom:1px solid #898C90; font-size: 18px; margin:0; ' +
			'padding-bottom:15px; color: #555555; font-weight: bold">' +
			`${d.properties.coc_number}: ${d.properties.COCNAME}</p><br>` +
			`<p style="color: #0071BC; margin: 0; font-size: 20px">Federal Funding: ${getDollarValue(
				d
			)}</p><br>` +
	...
	function handleBarChartMouseOver(d) {
		const html =
			'<p style="border-bottom:1px solid #898C90; font-size: 18px; margin:0; padding-bottom:15px"><b style="color:#555555">' +
			`${d.program_title}</b> [CFDA No. ${d.cfda_number}]</p><br>` +
			`<p style="color: #0071BC; margin: 0; font-size: 20px; padding:0">Federal Funding: ${formatNumber(
				d.fed_funding
			)}</p><br>` +
	...
	return (
		<div className="homeless-analysis federal-programs-section">
			<div className="homelessness-subheading">
			...
			<Grid container>
				<Grid item xs={12} md={6}>
					<div
						id="container2_1"
						className="homelessness-container2 homelessness-container2--left"
					/>
				</Grid>
				<Grid item xs={12} md={6}>
					<div
						id="container2_2"
						className="homelessness-container2 homelessness-container2--right"
					/>
				</Grid>
			</Grid>
			<Grid container>
