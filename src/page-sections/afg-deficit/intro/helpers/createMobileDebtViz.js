...
export default function createMobileDebtViz(config, height, width, dotsHeight) {
	...
	const priorYearDebtText = svg
		.append('g')
		.classed('prior-year-debt-balance', true)
		.attr(
			'transform',
			`translate(${(width * 0.7 - textWidth) / 2 - 10},${62 + dotsHeight})`
		);
	...
	d3.selectAll('.federal-debt-text>text').attr('y', (height - 35) * 0.5);
