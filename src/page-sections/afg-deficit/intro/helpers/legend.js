...
export function initMobileLegend(_config, debtTab) {
	config = _config || config;
	svg = establishContainer();

	const g = svg.append('g').classed('income-dot-legend-mobile', true);
	...
	if (debtTab) {
		const g2 = svg
			.append('g')
			.classed('federal-debt-legend-mobile', true)
