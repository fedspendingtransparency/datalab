		...
		if (out) {
			layers.master
				...
				.attr(
					...
					() =>
						`${translator(
							(chartWidth - chartWidth * scaleFactor) / 2,
							yOffset
						)} scale(${scaleFactor})`
				)
				.ease();
...
function initialDebtCompare(noDelay) {
	...
	layers.debt
		.transition()
		.delay(delay)
		.duration(() => (debtFirstTime ? duration : duration * 2))
...
function deficitOnly() {
	...
	layers.deficit.select('.legend').attr('opacity', 0);
}
