import SpendingData from '../../../../../static/americas-finance-guide/data/federal_spending_categories.csv';
...
function stackData(series) {
	...
	series.forEach((r, i) => {
		if (i === 0) {
			tracker = d3.min([
				0,
				series.filter(r => r.amount < 0).reduce((a, c) => (a += c.amount), 0),
			]);
		}
...
export function indexByYear(yyyy) {
	...
	getByYear(yyyy).forEach(r => {
		...
		if (r.child) {
			...
			// parent rows
			tempChildren = indexed[r.function][r.parent]
				? indexed[r.function][r.parent].subcategories
				: {};
