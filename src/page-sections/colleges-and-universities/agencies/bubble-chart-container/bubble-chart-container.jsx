...
const AgenciesBubbleContainer = props => {
	...
	const getClickedDetails = d => {
		if (!d) {
			...
			const summary = _data.allAgenciesRhpSummaryV2Csv.nodes.filter(
				i => i.subagency === d.name
			);
			...
			currentDetails = {
				...
				tables: [
					...
					{
						col1Title: `Investment Categories${
							Object.keys(invTop5).length > 5 ? ' (Top 5)' : ''
						}`,
						col2Title: 'Total Investment',
						rows: invTop5,
					},
					{
						col1Title: `Institutions${
							Object.keys(instTop5).length > 5 ? ' (Top 5)' : ''
						}`,
						col2Title: 'Total Investment',
						rows: instTop5,
					},
					...
					<Grid item xs={1}>
						<Hidden smDown>
							<div id="agencies-legend-colorkey">
								<div className={agenciesStyles.legendCirclekeyLabel}>
									<span>Agency</span>
								</div>
								<div className={agenciesStyles.legendCirclekeyLabel}>
								...
								<svg id={agenciesStyles.legendScalekey}>
									<circle
										r="25"
										className={agenciesStyles.legendScalekeyCircle}
										cx="60"
										cy="65"
									/>
									<circle
