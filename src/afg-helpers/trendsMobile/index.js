...
function buildSummary(row, d, config) {
	...
	const summary = row.append('div').classed('trend-row__summary', true),
		...
		percentChange = Number.parseFloat(Math.abs(difference / begin) * 100).toFixed(
			1
		);
	...
	summary
		.append('span')
		.classed('trend-row__summary-years trend-mobile__year', true)
		.text(summaryYearsString);
	summary
		.append('span')
		.classed('trend-row__summary-amounts trend-mobile__amount', true)
		.text(`${simplifyNumber(begin)} - ${simplifyNumber(end)} `);
	summary
		.append('span')
		.classed('trend-row__summary-change', true)
		.text(`${changeSign}${percentChange}%`);
}
...
function toggleVisibility() {
	...
	button
		.select('img')
		.attr('src', () => '/images/chart-' + (isActive ? 'white.svg' : 'green.svg'));
}
...
function toggleDetail(d) {
	...
	if (isActive) {
		...
		container = detail
			.append('div')
			.classed('trend-mobile trend-mobile--detail', true);
		trendMobile(
			d.subcategories.sort(sortByLatestYear),
			container,
			d.config,
			'detail'
		);
...
function buildChartButton(buttonRow) {
	...
	chartButton
		.append('img')
		.attr('src', '/americas-finance-guide/images/chart-white.svg');
	chartButton.on('click', toggleVisibility);
}

function buildDetailButton(buttonRow, d, config) {
	const detailButton = buttonRow
		.append('button')
	...
	detailButton
		.append('img')
		.attr('src', '/americas-finance-guide/images/arrow.svg');
	d.config = config;
