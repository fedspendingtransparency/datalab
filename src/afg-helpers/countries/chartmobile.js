...
const d3 = {
  select, selectAll, min, max, scaleLinear, axisBottom, transition, range,
};
...
function placeHorizontalStripes(dataLength) {
  ...
  const stripesContainer = svg.select('.cg-stripes-container');
  const stripes = stripesContainer.selectAll('line')
    .data(d3.range(dataLength + 1), (d) => d);
  ...
  stripes.enter()
    .append('line')
    .attr('x1', 0)
    .attr('y1', (d) => d * mobileDimensions.rowHeight + mobileDimensions.header + 3)
    .attr('x2', chartWidth)
    .attr('y2', (d) => d * mobileDimensions.rowHeight + mobileDimensions.header + 3)
...
function buildRow(d, i) {
  ...
  row.append('text').text((d) => d.country)
    .attr('y', 20);
...
function addMobileBarGroups(row, d) {
  const barElement = row.append('g')
    ...
    .attr('transform', (_, i) => translator(0, i * mobileDimensions.rowHeight + 10));
  ...
  barElement.append('text')
    .text(() => simplifyNumber(Math.abs(d[config.amountField])))
    .attr('font-size', 12)
