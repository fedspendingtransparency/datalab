...
export function drawChart(data, type, _config, detail, parentWidth) {
	...
	// Hack for IE, IE has a hard time getting the parent width
	if (typeof document !== "undefined" && !parentWidth) {
		const vizWrapper = document.getElementById("viz-wrapper")
		calculatedWidth = vizWrapper.clientWidth * 0.9
	}
	...
	if (detail) {
		...
		config.container = establishContainer(
			config.height,
			config.width,
			accessibilityAttrs
		)
		config.svg = config.container
			.append("g")
			.classed("pan-listen", true)
			.append("g")
			.classed("pan-apply", true)

		placeContainers(config, detail)

		d3.select("g.pan-listen")
			.call(
