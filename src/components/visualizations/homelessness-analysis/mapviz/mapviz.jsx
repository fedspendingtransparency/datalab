...
export default function Mapviz(props) {
	...
	function GenMap() {
		...
		d3.select('#map-svg')
			.append('desc')
			.attr('id', 'map-desc')
			.text('U.S. map showing homeless populations based on HUD Continuum of Care 2018 data. New York City had the highest concentration in the country, 78,676.');
	...
	function getColor(d) {
		for (let i = 0; i < tableData.length; i++) {
			if (d.properties.coc_number === tableData[i].coc_number) {
				if (tableData[i].total_homeless <= 0) { return ('#EEEEFF'); } if (tableData[i].total_homeless <= 200) { return ('#E1E1F4'); } if (tableData[i].total_homeless <= 300) { return ('#D6D6EA'); } if (tableData[i].total_homeless <= 500) { return ('#D0D0F0'); } if (tableData[i].total_homeless <= 700) { return ('#B8B8D1'); } if (tableData[i].total_homeless <= 1000) { return ('#ACACC6'); } if (tableData[i].total_homeless <= 1500) { return ('#9E9EBA'); } if (tableData[i].total_homeless <= 2000) { return ('#9595AD'); } if (tableData[i].total_homeless <= 2500) { return ('#8888A5'); } if (tableData[i].total_homeless <= 3000) { return ('#7F7FA0'); } if (tableData[i].total_homeless <= 3500) { return ('#72729B'); } if (tableData[i].total_homeless <= 4000) { return ('#656593'); } if (tableData[i].total_homeless <= 5000) { return ('#5E5D91'); } if (tableData[i].total_homeless <= 6000) { return ('#534E89'); } if (tableData[i].total_homeless <= 7000) { return ('#4D4484'); } if (tableData[i].total_homeless <= 8000) { return ('#45387A'); } if (tableData[i].total_homeless <= 12000) { return ('#392870'); } if (tableData[i].total_homeless <= 80000) { return ('#311C66'); }
	...
	return (
		<div className={props.display ? '' : 'hidden'}>
			...
			<div className={styles.homelessPanelOneInfo}>
				<p>
					* White areas with no hover-over represent areas of the country that are not represented by a Continuum of
					Care area.
				</p>
