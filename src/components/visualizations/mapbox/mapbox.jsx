...
export default function Mapbox(props) {
	...
	useEffect(() => {
	  ...
	  // set opactiy to 40%
	  map.setPaintProperty('clusters', 'circle-opacity', 0.35);
	  ...
	  map.loadImage('/mapbox/map-pin.png', (err, image) => {
	    if (err) throw err;
	  ...
	  map.on('click', 'clusters', (e) => {
	    ...
	    const { coordinates } = cluster[0].geometry;
	  ...
	  map.on('mouseenter', 'unclustered-point', (e) => {
	    ...
	    const name = e.features[0].properties.Recipient;
	    const state = e.features[0].properties.State;
	    const fedInvest = formatNumber('dollars', e.features[0].properties.Total_Federal_Investment);
	    const county = e.features[0].properties.COUNTY;
	    const numStudents = formatNumber('number', e.features[0].properties.Total);

	    const tooltipHtml = `<div class='tooltip-float'><p class='map-tooltip-p-left-inst'>Institution</p> <p class='map-tooltip-p-right'>${name}</p></div> <div class='tooltip-float'><p class='map-tooltip-p-left'>State</p> <p class='map-tooltip-p-right'>${state}</p></div><div class='tooltip-float'><p class='map-tooltip-p-left'>County</p> <p class='map-tooltip-p-right'>${county}</p></div><div class='tooltip-float tooltip-float--underline'><p class='map-tooltip-p-left'>Number of Students </p> <p class='map-tooltip-p-right'>${numStudents}</p></div><div class='tooltip-float'><p class='map-tooltip-p-left'>Total $ Received</p><p class='map-tooltip-p-right-invest'>${fedInvest}</p></div>`;
	  ...
	  map.on('click', 'schools', (e) => {
		...
		function filterMapSearch() {
			$('#map-search__input').keyup(() => {
	  let input; let filter; let ul; let li; let i; let
					txtValue;
	...
	return (
		<>
			<div id="collegesMap" aria-label={props.altText} />
			<div id="inst-panel" />
		</>
	);
}
