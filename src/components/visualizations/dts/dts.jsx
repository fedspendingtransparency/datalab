...
function DTS(props) {
	useEffect(() => {
		...
		const margin = {
			top: 20, right: 20, bottom: 110, left: 60,
		};
		const margin2 = {
			top: 330, right: 20, bottom: 30, left: 60,
		};

		let allOptions; let optionsDict; let lastDate; let
			categorySeparatorDate;
		let data; let optionsData; let dateScaleValues; let allToSpending; let categoryToSpendingPrevFY; let
			todayAllCategorySpending;
      ...
      ;
		...
		let width; let height; let height2; let
			zoom;
		...
		function updateCustomGrabbers(s) {
			...
			if (s == null) {
				...
				handle.attr('display', null).attr('transform', (d, i) => `translate(${s[i]},${height2 / 2})`);
			}
		}
		...
		function updateHistoryWithNewFrequencyOrCategory() {
			...
			const theQueryString = `?${serialize({
				start, end, frequency, category,
			})}`;
			history.replaceState(null, '', theQueryString);
		}

		function updateHistoryWithNewBrush(s) {
			const start = new Date(x2.invert(s[0])).toISOString().slice(0, 10).replace(/-/g, '');
			const end = new Date(x2.invert(s[1])).toISOString().slice(0, 10).replace(/-/g, '');
			const frequency = d3.select('#frequency-selector').property('value');
			// let category = d3.select('#category-selector').property('value') || 'All Categories';
			const category = 'All Categories';
		...
		function createBarChart(yearToSpendingArray, redraw) {
			...
			const margin = {
				top: 20, right: 20, bottom: 50, left: 150,
			};
		...
		function createTable(yearToSpendingArray) {
			...
			const table = d3.select('.svg-tsbfy-container').append('table').attr('class', 'tsbfy-table');
			...
			const totalSpending = tableData.map((d) => d.spending).reduce((a, b) => a + b, 0);
		...
		function createSelect(condensedOptions, activeOptions, inactiveOptions) {
			...
			const select = d3.select('#category-selector').on('change', onCategoryChange);
        ...
        ;
			...
			const mouseOverDataPoint = svg.append('g')
			...
			function mouseMove() {
				...
				const d = getMouseOverDataPoint(masterMapping[categoryValue][frequencyValue], this);
				...
				const todayDataPoint = dollarFormatter(getMouseOverDataPoint(masterMapping[categoryValue].today, this).value);
				const mtdDataPoint = dollarFormatter(getMouseOverDataPoint(masterMapping[categoryValue].mtd, this).value);
				const fytdDataPoint = dollarFormatter(getMouseOverDataPoint(masterMapping[categoryValue].fytd, this).value);
		...
		function drawChart() {
			...
			if (data) {
				...
				const condensedOptions = Object.keys(categoryToSpendingPrevFY).sort((a, b) => categoryToSpendingPrevFY[b] - categoryToSpendingPrevFY[a]).slice(0, 10);
				...
				const yearToSpendingArray = getYearToSpendingArray(optionsDict['All Categories'].fytd);
			...
			<div className="dts-viz-container">
				...
				<div className="viz-tsbfy-container">
					...
					<div className="svg-tsbfy-container">
						<svg id="viz-tsbfy-wrapper" width="750" height="500" viewBox="0 0 750 500" />
					</div>
				</div>
			</div>
