...
export default function Glossary(props) {
  ...
  function createSVGElements() {
    ...
    const closeButtonSvg = closeButton
        .append('svg')
        .classed('cg-icon-close', true)
        .attr('viewBox', '0 0 512 512')
        .attr('aria-label', 'Close Glossary 123'),
      searchBoxSvg = searchBox
        .append('svg')
        .classed('cg-icon-search', true)
        .attr('viewBox', '0 0 512 512')
        .attr('aria-label', 'search'),
      backButtonSvg = backButton
        .append('svg')
        .classed('cg-glossary-angle-left', true)
        .attr('viewBox', '0 0 512 512')
    ...
    closeButtonSvg.append('title').text('Close Glossary');
    closeButtonSvg
      .append('g')
      .append('path')
      .attr(
        'd',
        'M256 212L50 6 6 50l206 206L6 462l44 44 206-206 206 206 44-44-206-206L506 50 462 6'
      );
    ...
    backButtonSvg.append('title').text('Back');
    backButtonSvg
      .append('g')
      .append('path')
  ...
  function createTermHTMLElements(terms, searchTerm) {
    ...
    Object.keys(filteredTerms).forEach(function (t) {
      ...
      hashSection = termResultList
        .append('div')
        .classed('cg-glossary-result-group', true);
      hashSection
        .append('h2')
        .classed('cg-group-title', true)
        .html(t);
  ...
  function showIndividualTerm(termsArr, termDisplay) {
    ...
    if (term.url_path) {
      const wrapperDiv = definitionWrapper
        .append('div')
        .classed('cg-glossary-resources', true);
  ...
  function resizeTermListDiv() {
    ...
    if (!termListDiv.empty()) {
      ...
      const parentDiv = termListDiv.node().parentElement,
        siblingDiv = parentDiv.getElementsByClassName(
          'cg-glossary-header-wrapper'
        )[0];

      termListDiv.style(
        'height',
        parentDiv.clientHeight - siblingDiv.clientHeight + 'px'
      );
    }
  }
  ...
  function addGlossaryEvents() {
    ...
    searchTextBox
      .on('propertychange', searchBoxEvent)
          ...
          <div className="cg-glossary-header-wrapper">
            <div className="cg-glossary-header">
              <div role="navigation" aria-label="Glossary navigation">
                <button
                  className="cg-close-button"
                  id="cg-glossary-close-button"
                  aria-label="Close Glossary"
                  title="Close Glossary"
                />
              </div>
              <h1 id="cg-glossary-title" className="cg-glossary-title"
                  tabIndex="-1">
                Glossary
              </h1>
              <div className="cg-glossary-search-bar">
                <input
                  id="cg-search-text-box"
                  type="text"
