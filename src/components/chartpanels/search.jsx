...
export default class Search extends React.Component {
  ...
  */
	...
	constructor(props) {
		...
		let initItem = '';
		props.initItem &&
			(initItem = this.props.searchList.find(e => e.id === props.initItem)
				.display);
	...
	handleKeyPress = (e, index) => {
		if (e.key === 'Enter' || e.key === ' ') {
			...
			(e.key === 'ArrowDown') &
			(index !== this.state.filteredList.length - 1)
		) {
			e.preventDefault();
			const nextElement = document.getElementById(
				`${this.props.listId}-list-row-${index + 1}`
			);
			nextElement.focus();
			...
			const elementId = e.shiftKey
				? `${this.props.listId}-search-bar`
				: `${this.props.listId}-search-button`;
	...
	filterBoxIcon = () => (
		<InputAdornment position="end">
			<IconButton aria-label={this.state.icon} onClick={this.clickIcon}>
