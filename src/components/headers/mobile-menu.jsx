...
class MobileMenu extends React.Component {
	...
	returnActiveList = (data, name) => {
		return data.map((item, key) => {
			...
			return (
				<>
					<li
						className={styles.dataListLi}
						key={key}
						onKeyDown={e => this.sublistTab(e, name)}>
						<a id={id} href={item.link} tabIndex="0" className={styles.dataListA}>
							{item.name}
						</a>
					</li>
					<hr className={styles.mobileHr} />
	...
	sublistTab = (e, name) => {
		...
		if (e.key === 'Tab') {
			if (
				(document.activeElement.id === 'menu-first-item' && e.shiftKey) ||
				(document.activeElement.id === 'menu-last-item' && !e.shiftKey)
			) {
				if (name === 'Analyses') {
					this.handleClick('Analyses');
				} else if (name === 'Resources') {
	...
	handleExit = e => {
		if (e.key === 'Tab') {
			if (
				(document.activeElement.id === 'analysesBtn' && e.shiftKey) ||
				(document.activeElement.id === 'glossaryBtn' && !e.shiftKey)
			) {
				...
				<div>
					<ul
						className={`${styles.mobile} ${
							this.state.isShowing ? '' : styles.hidden
						}`}
						...
						<div>
							<li
								...
								data-id="0"
								onClick={() => this.handleClick('Analyses')}>
							...
							<ul
								...
								className={`${styles.toggleList} ${
									this.state.analysesCheck ? '' : ` ${styles.hidden}`
								}`}>
								{this.returnActiveList(this.state.data[0].analyses, 'Analyses')}
							</ul>
						</div>
						<div>
						...
						<div>
							...
							<ul
								id="resources"
								className={`${styles.toggleList} ${
									this.state.resourcesCheck ? '' : ` ${styles.hidden}`
								}`}>
								{this.returnActiveList(this.state.data[3].resources, 'Resources')}
							</ul>
						</div>
						<div>
						...
						<div>
							<li
								className={`${styles.item} ${styles.glossary}`}
								data-id="5"
								id="glossary"
								onClick={() => this.handleClick('Glossary')}>
